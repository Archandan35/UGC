...
export default function ResultPage() {
  ...
  // REAL analytics only — no fabricated/random numbers (audit P0 fix).
  // Per-question time comes from the exam engine; comparative stats (topper time,
  // percentile) are shown only when real aggregated data is provided, else "—".
  const timePerQuestion = result.timePerQuestion || {};
  const userTime = current?.id ? Number(timePerQuestion[current.id] || 0) : 0;
  const questionAccuracy =
    totalQuestions > 0 ? Math.round((correctCount / totalQuestions) * 100) : 0;
  const topperTime =
    current?.id && result.topperTime ? result.topperTime[current.id] ?? null : null;
  const performance = result.percentile ?? null;
    ...
    <div className="page">
        ...
        <div className="review-actions">
          ...
          <button
            ...
            onClick={() => generateCertificate({
              ...
              examTitle: result.examId || "Mock Exam",
              score: Number(obtainedMarks),
              totalMarks,
              accuracy: questionAccuracy,
              certificateId: result.id || undefined,
            })}
          >
            Certificate
          </button>
      ...
      <div className="review-analytics-strip">
        ...
        <div className="analytics-mini-card">
          ...
          <h3>{topperTime == null ? "\u2014" : formatTime(topperTime)}</h3>
        </div>
        ...
        <div className="analytics-mini-card">
          ...
          <h3>{performance == null ? "\u2014" : `Top ${100 - performance}%`}</h3>
        </div>
      </div>
