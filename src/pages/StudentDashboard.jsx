...
import {
  subscribeExams, subscribeSubjects, subscribeTopics, subscribeSubtopics,
} from "../data-layer";
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeExams((data) => setExams(data));
  ...
  useEffect(() => {
    ...
    const unsub = subscribeTopics((data) => setTopics(data));
  ...
  useEffect(() => {
    ...
    const unsub = subscribeSubtopics((data) => setSubTopics(data));
        ...
        {filteredAndSortedExams.length === 0 ? (
          ...
          filteredAndSortedExams.map((exam) => (
            ...
            <div key={exam.id} className="subject-card glass-card">
              ...
              {/* ── Dual action buttons: Resume + Restart (Issue 2) ── */}
              <div className="exam-card-actions">
                {(() => {
                  const attemptKey = getAttemptKey(exam.id, null, null);
                  const canResume  = hasAttemptState(attemptKey);
                  return (
                    <>
                      <button
                        className={`exam-card-btn exam-card-btn-resume${canResume ? "" : " exam-card-btn-disabled"}`}
                        disabled={!canResume}
                        onClick={() => canResume && navigate(`/exam/${exam.id}?start=resume`)}
                        title={canResume ? "Resume your previous attempt" : "No saved attempt to resume"}
                        aria-disabled={!canResume}
                      >
                        ▶ Resume
                      </button>
                      <button
                        className="exam-card-btn exam-card-btn-restart"
                        onClick={() => navigate(`/exam/${exam.id}?start=restart`)}
                        title="Start a fresh attempt from Q1"
                      >
                        ↺ Restart
                      </button>
                    </>
