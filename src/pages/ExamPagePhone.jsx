...
export default function ExamPagePhone() {
  ...
  /* Back-navigation: show leave dialog instead of blank screen */
  useEffect(() => {
  ...
  /* Show cheat dialog on cheatCount change */
  useEffect(() => {
    if (cheatCount > 0) {
  ...
  const handleLeaveExam     = useCallback(() => { setShowLeaveDialog(false); navigate("/exams"); }, [navigate]);
  const handleSaveAndLeave  = useCallback(() => { setShowLeaveDialog(false); navigate("/exams"); }, [navigate]);
          ...
          <div className="ph-loading-logo">
            <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="24" cy="24" r="22" stroke="#2563eb" strokeWidth="3" fill="none"
                strokeDasharray="5 3" className="ph-loading-ring" />
      ...
      {showCheatDialog && (
        <div className="ph-dialog-overlay ph-cheat-overlay">
          <div className="ph-dialog ph-cheat-dialog">
            ...
            <button
              className="ph-dialog-btn ph-dialog-btn-continue ph-cheat-ok-btn"
      ...
      <div className="ph-header">
        ...
        <div className="ph-header-row1">
          <div className="ph-title-wrap">
            <div className="ph-title">{examData?.name || examData?.title || "Exam"}</div>
          </div>
        </div>

        {/* Row 2: Pause | Warning Counter | spacer | Timer */}
        <div className="ph-header-row2">
          ...
          <div
            className={timerWarning ? "ph-timer warning" : "ph-timer"}
          >
            {formatTime(timeLeft)}
          </div>
        </div>
        ...
        <div className="ph-question-card">
          ...
          <div className="ph-options">
            {options.map((option, idx) => (
              <label
                key={idx}
                className={selectedOpt === idx ? "ph-option ph-option-selected" : "ph-option"}
              >
                <input
                  type="radio"
      ...
      <div className={sidebarOpen ? "ph-sidebar open" : "ph-sidebar"}>
        ...
        <div className="ph-sidebar-header">
          ...
          <div className="ph-sidebar-top-row">
            <span className="ph-sidebar-title">
              {examData?.name || examData?.title || "Question Palette"}
            </span>
            <button className="ph-close-btn" onClick={() => setSidebarOpen(false)}>
              ✕
            </button>
          </div>
        ...
        <div className="ph-sidebar-footer">
          ...
          <div className="ph-sidebar-footer-row">
            <button className="ph-footer-btn paper" onClick={() => setSidebarOpen(false)}>
              📄 Question Paper
            </button>
            <button className="ph-footer-btn instructions" onClick={() => setSidebarOpen(false)}>
              ℹ️ Instructions
            </button>
          </div>
