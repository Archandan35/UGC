...
import {
  getCurrentUid, subscribeUserByUid, subscribeUserResults,
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (!uid) return;
    return subscribeUserByUid(uid, (userData) => {
      if (userData) setUserRole(userData.role || "student");
    });
  }, []);
  ...
  useEffect(() => {
    const uid = getCurrentUid();
    if (!uid) return;
    return subscribeUserResults(uid, (data) => {
  ...
  useEffect(() => subscribeSubtopics(setSubTopics), []);
