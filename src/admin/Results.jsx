...
import { subscribeResults, subscribeSubjects, subscribeTopics, subscribeSubtopics, subscribeUsers } from "../data-layer";
...
export default function Results() {
  ...
  useEffect(() => {
    const subs = [
      subscribeResults((data) => {
        const sorted = [...data].sort((a, b) => safeNum(b.score) - safeNum(a.score));
        setResults(sorted);
      }),
      subscribeSubjects(setSubjects),
      subscribeTopics(setTopics),
      subscribeSubtopics(setSubTopics),
  ...
  function exportPDF() {
    ...
    docPDF.text("Results Report", 14, 20);
    autoTable(docPDF, {
      startY: 30,
      head: [["Rank","User","Subject","Topic","Score","Accuracy"]],
      body: leaderboard.map((r, i) => [
        i + 1, resolveUser(r), resolveSubject(r), resolveTopic(r),
        safeNum(r.score), safeNum(r.accuracy) + "%",
      ]),
    });
    docPDF.save("results-report.pdf");
  }

  return (
    <AdminLayout>
      <div className="page-header">
        <div><h2>Results Analytics</h2><p>Total Attempts: {totalAttempts}</p></div>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={subjectPerformance}>
              <XAxis dataKey="subject" /><YAxis /><Tooltip />
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={accuracyData} dataKey="value" outerRadius={100} label>
                {accuracyData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
              </Pie>
              <Tooltip />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div className="table-card">
        ...
        <div className="um-table-wrapper">
          <table className="data-table">
            <thead><tr><th>Rank</th><th>User</th><th>Subject</th><th>Topic</th><th>SubTopic</th><th>Score</th><th>Accuracy</th></tr></thead>
            <tbody>
              {leaderboard.map((r, i) => (
                <tr key={r.id}>
                  <td>#{i + 1}</td><td>{resolveUser(r)}</td><td>{resolveSubject(r)}</td>
                  <td>{resolveTopic(r)}</td><td>{resolveSubTopic(r)}</td>
                  <td>{safeNum(r.score)}</td><td>{safeNum(r.accuracy)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="table-card">
        ...
        <div className="um-table-wrapper">
          <table className="data-table">
            <thead><tr><th>Date & Time</th><th>User</th><th>Subject</th><th>Topic</th><th>SubTopic</th><th>Correct</th><th>Wrong</th><th>Score</th><th>Accuracy</th></tr></thead>
            <tbody>
              {results.length === 0 ? (
                ...
                <tr key={r.id}>
                  ...
                  <td>{resolveUser(r)}</td><td>{resolveSubject(r)}</td>
                  <td>{resolveTopic(r)}</td><td>{resolveSubTopic(r)}</td>
                  <td>{r.correct ?? 0}</td><td>{r.wrong ?? 0}</td>
                  <td>{safeNum(r.score)}</td><td>{safeNum(r.accuracy)}%</td>
                </tr>
              ))}
            </tbody>
