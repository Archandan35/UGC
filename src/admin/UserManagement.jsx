...
/*
 * deleteUserCompletely is imported from "../data-layer" above.
 * It removes the user profile, all related collection docs,
 * and defers auth deletion to the
 * server-side Cloud Function (or handles it directly if the admin
 * is deleting their own account).
 */
...
export default function UserManagement() {
  ...
  const validUsers   = users.filter(u => u.uid || u.id); // must have an auth uid
  ...
  async function handleAddUser() {
    ...
    try {
      /* ── CASE 1: Check if this email already has a user profile ──
         If a profile already exists for this email, show a clear
      ...
      */
      ...
      /* ── CASE 2: Try to create the Auth account ──
         If auth/email-already-in-use is thrown, the email exists in Auth
         but has NO user profile (orphaned Auth account).
         We recover by creating the user profile linked to the existing uid
         using fetchSignInMethodsForEmail to confirm it exists, then
         creating only the user profile.
      */
      let uid;
      try {
        // NOTE: Supabase does not sign out the current user when creating another account
        // via the admin API, so no secondary client is needed.
        const secondaryAuth = getSecondaryAuth();
        const cred = await createUserWithEmailAndPassword(emailLower, addForm.password);
        uid = cred.user.id;
        // Clear the secondary session immediately so it never lingers.
        await signOutAuth().catch(() => {});
      } catch (authErr) {
        if (authErr.code === "auth/email-already-in-use") {
          /*
           * The email exists in Auth but has no user profile.
           * We cannot retrieve the uid from the client without the user
           * logging in. Show a clear actionable message.
           */
          setAddError(
            `This email is already registered in Auth but has no profile in the system. ` +
            `To fix this: ask the user to log in once so their profile is auto-created, ` +
            `or delete the orphaned Auth account from the Auth console and try again.`
          );
          setAddLoading(false);
          return;
        }
      ...
      /* ── CASE 3: Auth account created — now create user profile ── */
      ...
      /* Translate common auth error codes into plain English */
      const code = e.code || "";
  ...
  async function handleEditUser() {
    ...
    try {
      ...
      if (roleChanged) {
        const { logAdminAction } = await import("../services/auditLogService");
        await logAdminAction("user.role_changed", {
          targetUid: editUser.id,
          from: editUser?.role,
          to: editForm.role,
        });
      }
  ...
  async function handleSetPassword() {
    ...
    setSetPwdError("");
    try {
      if (getAuth()?.currentUser?.uid === setPasswordUser.uid) {
        await updatePassword(getAuth().currentUser, newPassword);
        closeSetPasswordModal();
      } else {
        // Requires Admin SDK on the server — send reset email as fallback
        await sendPasswordResetEmail(setPasswordUser.email);
        setSetPwdError(
          "Direct password changes for other users require a server-side Admin API call. " +
  ...
  async function saveAccess() {
    ...
    try {
      await updateUser(accessUser.id, {
