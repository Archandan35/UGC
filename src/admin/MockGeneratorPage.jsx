...
export default function MockGeneratorPage(){
  ...
  useEffect(()=>{
    ...
    const u4 = subscribeExams(setExams);
    getQuestions().then(setQuestions);
    return ()=>{ u1(); u2(); u3(); u4(); };
  },[]);

  /* EXAMS — duplicate useEffect removed; already subscribed in DATA LOADING above */
  ...
  */
  ...
  async function handleGenerate(){
    ...
    try{
      ...
      console.error(error);
      alert("Failed to generate mocks: " + (error?.message || String(error)));
    }finally{
      setLoading(false);
    }
